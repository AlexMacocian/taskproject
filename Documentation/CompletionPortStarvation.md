...
This scenario demonstrates what happens when all IOCP threads are blocked and
async I/O operations cannot complete.
...
When all IOCP threads are blocked, async I/O completions queue up waiting for
a thread to process them.
...
Restricts IOCP threads to `Environment.ProcessorCount`
(e.g., 8 threads on an 8-core machine).
...
Queues work directly to the IOCP pool using native overlapped I/O -
this bypasses the worker pool entirely.

### 3. Block All IOCP Threads

```C#
static unsafe void IOCompletionCallback(
  uint errorCode,
  uint numBytes,
...
- `File.ReadAllTextAsync` initiates an async read (completes almost instantly
at the OS level)
...
Even though the actual file I/O is instant, **IOCP starvation delays the
completion callback**. This can cause unexpected latency in I/O-heavy
applications when IOCP threads are blocked by:
